...
- 날짜/시간 처리, 포맷, 비교, 상대 시간 계산, 날짜 더하기/빼기 같은 로직은 원칙적으로 `dayjs`를 사용한다.
- 단순한 timestamp 저장/전달처럼 계산이나 표시가 전혀 없는 경우에만 기본 `Date` 또는 숫자 값을 그대로 유지할 수 있다.
- `new Date()`는 외부 입력을 즉시 `dayjs`로 변환하기 위한 최소 용도로만 허용하고, 이후 로직은 가능하면 `dayjs` 기준으로 통일한다.
...
- `null`/`undefined` 가능성이 있는 값은 가능하면 명시적으로 처리한다.
- nullish만 구분하려는 의도라면 가능하면 `lodash`의 `isNil` / `!isNil` 형태를 우선 검토한다.
- 예를 들어 `value === null || value === undefined` 같은 반복 패턴은 가능하면 `isNil(value)`처럼 의도가 바로 드러나는 형태를 선호한다.
- 다만 빈 문자열, `0`, `false`까지 함께 다루는 falsy 분기는 굳이 `isNil`로 치환하지 않는다.
- 타입과 문맥상 이미 안전한 값에 대해서는 불필요한 nullish 체크를 강제하지 않는다.
...
- renderer, main, preload를 함께 쓰는 공용 유틸은 번들/호환성까지 고려해서 선택하고, 단순히 tree-shaking만 이유로 무리하게 라이브러리를 교체하지 않는다.
...
- 이 저장소에서 Codex가 제안하거나 작성하는 커밋 메시지의 기본 언어는 한국어다.
- 사용자가 영어 커밋 메시지를 명시적으로 요청한 경우를 제외하고, 영어 커밋 메시지를 기본값이나 초안으로 제안하지 않는다.
- 영어 커밋 메시지를 먼저 쓰고 나중에 한국어로 바꾸는 방식도 사용하지 않는다.
- 커밋 메시지는 반드시 `type: 한국어 요약` 형식을 따른다.
  - 예: `feat: 다운로드 설정 초기값 정리`
  - 예: `fix: 플레이어 다음 곡 이동 오류 수정`
  - 예: `refactor: settings validator 구조 정리`
- 허용하는 type은 아래 범위로 제한한다.
  - `feat`, `fix`, `refactor`, `chore`, `docs`, `style`, `test`, `build`, `ci`
- 커밋 제목은 한 줄로 짧고 직접적으로 작성한다.
- 커밋 제목은 사용자 관점 또는 변경 의도가 바로 보이게 작성한다.
- `update`, `modify`, `change`, `misc`, `cleanup`처럼 의미가 약한 영어 단어만으로 제목을 쓰지 않는다.
- `fix: 수정`, `feat: 작업`, `chore: 정리`처럼 지나치게 모호한 한국어 제목도 쓰지 않는다.
- 파일명, 함수명, 라이브러리명, IPC 채널명, 설정 키처럼 고유 식별자는 원문 영문 그대로 유지할 수 있다.
- 본문이 필요하면 한국어로만 작성하고, 변경 이유와 영향 범위만 짧게 적는다.
- 여러 변경이 섞여 있어도 커밋 제목은 대표 변경 하나를 기준으로 한국어로 요약한다.
- 커밋 메시지에서 불필요한 감탄문, 홍보성 표현, 장황한 설명은 쓰지 않는다.
