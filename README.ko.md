...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/release-1.0.12-111827" alt="Release 1.0.12" />
...
- 최근 URL 기록 저장
- 완료 항목 보관함에서 정렬, 열기, 위치 열기, 삭제, 썸네일, sidecar 메타데이터 재사용 지원
- 로컬 비디오/오디오 재생용 내장 플레이어
- 탐색, 볼륨, 음소거, 재생 속도, 전체화면, 오디오 시각화, ambient particles 컨트롤
- 테마 모드, 모드별 테마 스타일, 언어, 기본 다운로드 타입, 플레이리스트 제한, 보관함 정렬 설정
...
- Node.js LTS 권장 (가능하면 Node 20 이상)
- Corepack으로 pnpm 사용 권장 (`corepack enable`)
...
일반 설치와 세팅은 `pnpm install --frozen-lockfile` 기준으로 유지합니다.
의존성을 실제로 바꿀 때만 `pnpm add` 또는 `pnpm update`를 사용하고, 변경된 lockfile을 함께 커밋합니다.
