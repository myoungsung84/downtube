...
## LLM (Experimental / 실험적)

LLM 관련 인프라와 스크립트는 Experimental(실험적) 단계입니다. 특히 LLM 커밋 워크플로는 실험적이며 변경 가능성이 높습니다.

### 구성

- llm/compose.yml: LLM 서비스용 Docker Compose 정의
- llm/models/: 로컬 모델 파일 저장 위치 (git에서 제외)
- scripts/llm/llm-up.sh: LLM 인프라 기동 스크립트
- scripts/llm/llm-down.sh: LLM 인프라 종료 스크립트
- scripts/llm/llm-diff-commit.mjs: LLM 커밋 도우미 (실험적)

### 의존성

- Docker Desktop 설치 및 실행 필요 (Docker Compose v2 포함)
- LLM 서버는 llama.cpp Docker 이미지 기반으로 동작
- 스크립트 실행을 위해 `bash` 및 `curl` 필요
...
# 또는 직접 실행
bash scripts/llm/llm-up.sh
bash scripts/llm/llm-down.sh
```

### LLM 커밋 (Experimental / 실험적)
