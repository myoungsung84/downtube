...
# -------------------------
# Resolve paths (this file lives in llm/)
# -------------------------
...
trim_cr() {
  # remove trailing CR (CRLF issue)
  printf '%s' "$1" | sed -e 's/\r$//'
}

# Read a single KEY from .env safely (no source)
# - ignores blank lines and comments
# - supports: KEY=value (no export)
# - keeps everything after first '=' (so URLs with '=' are OK)
env_get() {
  local key="$1"
  local file="$2"
  [ -f "$file" ] || return 0
...
# Default getter: prefer env var already set, else .env, else default
getv() {
  local key="$1"
  local def="${2:-}"
  local val="${!key:-}"
...
compose() {
  # always use compose.yml under llm/ and env-file llm/.env
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" "$@"
}

detect_profile() {
  # cpu|gpu|auto (auto tries GPU if possible)
  local p
  p="$(getv LLM_PROFILE cpu)"
  case "$p" in
    cpu|gpu) echo "$p" ;;
    auto)
      # auto: if docker gpu works then gpu else cpu
