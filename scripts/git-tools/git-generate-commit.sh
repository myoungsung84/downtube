...
# ============================================================
# git-generate-commit.sh (STAGED ONLY)
#
# LLM Git Commit Message Generator (llama.cpp server) - Bash + jq
#
# - Conventional Commit style message
# - INPUT BUNDLE: staged diff only + untracked list only
# - Prompts loaded from files (system/user)
#
# Usage:
#   bash scripts/git/git-generate-commit.sh
#   bash scripts/git/git-generate-commit.sh --show-diff
#   bash scripts/git/git-generate-commit.sh --system <path> --user <path>
#   bash scripts/git/git-generate-commit.sh --out .git/COMMIT_EDITMSG.llm
#
# Notes:
# - This script DOES NOT stage files.
# - It generates message based on *staged changes only*.
# - Untracked files are listed by path only (no content).
# ============================================================
...
usage() {
  cat <<'EOF'
Usage:
  git-generate-commit.sh [--show-diff] [--system <path>] [--user <path>] [--out <file>]

Flags:
  --show-diff   Print the bundle that will be sent to LLM (truncated)
  --system <p>  System prompt path (default: scripts/prompts/commit.system.txt)
  --user <p>    User prompt path   (default: scripts/prompts/commit.user.txt)
...
if ! check_health; then
  ...
  echo "TIP: Start it first: npm run llm up"
  exit 1
fi
